...
<Block as="h2" ss="margin-top: 1rem;" textgradient="accent">1. The default export is ready to use</Block>
...
import Button from "@techytools/ui/components/content/Button";
...
<Block as="h2" ss="margin-top: 5rem;" textgradient="accent">2. Alternatively, use the provided styled function</Block>
...
<Code
  ss="font-size: 0.85rem;"
  code={`import { withButton } from "@techytools/ui/components/Button";
...
export const MyButton = withButton({
  ...
  \`,
  ssDesktop: \`
    &:hover { box-shadow: 1px 2px 1px var(--color-cta); }
  \`,
...
<Block as="h2" ss="margin-top: 5rem;" textgradient="accent">3. Or whatever you prefer</Block>

Standard `style` attribute, Styled Components, or the `css` prop from `@emotion/react`.
