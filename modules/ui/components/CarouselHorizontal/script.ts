...
export default class CarouselHorizontal {
  /*
  ...
  constructor(carousel) {
    if (!carousel || !carousel.querySelectorAll) {
      ...
      console.warn('CarouselHorizontal: carousel element not found');
      return;
    }
...
if (typeof window === 'object') {
  window.horizontal_carousel = CarouselHorizontal;
  window.horizontal_carousels = function (elements) {
    const refs = [];
    if (elements && elements.length) {
      for (const el of elements) {
        refs.push(new CarouselHorizontal(el));
      }
    }
    return refs;
  };
