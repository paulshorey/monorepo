...
export const reset_failedSource = function (name) {
  global.pgdb.query(`UPDATE crawl.sources SET failed_count = 0, words_affected='[]' WHERE name='${name}'`)
}
...
export const add_word = function (row) {
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      let res: any = await global.pgdb.query(queryString)
      if (res && res.rows && res.rows.length) {
        // ignore
        resolve("")
        return
      ...
      if (lowercase_row && uppercase_row) {
        ...
        if (lowercase_row.list_count > uppercase_row.list_count) {
          ...
          await global.pgdb.query(
            `UPDATE crawl.words SET key=e'${lowercase_row.key}' WHERE key=e'${uppercase_row.key}'`
          )
        ...
        await global.pgdb.query(`UPDATE crawl.words SET key=e'${key}' WHERE key=e'${query.key}'`)
