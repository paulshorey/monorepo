...
export default function (string, options: any = {}) {
  return new Promise(function (resolve) {
    ...
    setImmediate(function () {
      // start
      if (DEBUG1) global.cconsole.info(`spellcheck input "${string}"`)
      ...
      try {
        ...
        // send request
        if (DEBUG2) global.cconsole.log("send response", data)
        axios
          .get(url, {
            params: data,
            headers: headers,
          ...
          .then(function (response) {
            if (DEBUG2) global.cconsole.log("got response", response.data)
            // response data
            if (response.data.flaggedTokens[0]) {
              ...
              if (DEBUG1) global.cconsole.log("bing suggestions", suggestions)
            ...
            if (DEBUG1) {
              if (string_spellchecked && string_spellchecked !== string) {
                ...
                if (DEBUG1) global.cconsole.info(`bing spellcheck did not fix "${string}"`)
              }
            }
          ...
          .catch(function (error) {
            if (DEBUG1) global.cconsole.warn(`bing spellcheck failed`, error)
            clearTimeout(timeout)
            resolve(string)
          })
      } catch (err) {
        if (DEBUG1) global.cconsole.error(`bing spellcheck error on "${string}"`, err)
      }
    })
  })
}
