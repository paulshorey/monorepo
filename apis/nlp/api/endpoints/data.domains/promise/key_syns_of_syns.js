...
import { data_word_get_parsed, data_word_get } from "../../data.words/pgdb";
import { data_domain_get_parsed } from "../pgdb";
/**
...
export default function (key) {
    return new Promise(async (resolve) => {
        let syns_of_syns = {};
        if (DEBUG2)
            global.cconsole.log(`key_syns_of_syns key="${key}"`);
        try {
            /*
             * Get row from DB
             */
            let row = await data_domain_get_parsed(key, "key, syns1");
            if (!row || !row.syns1) {
                if (DEBUG1)
                    global.cconsole.warn(`!row || !row.syns1 for key="${key}"`);
                resolve({});
                return;
            }
            if (DEBUG2)
                global.cconsole.log(`row="${row.key}" syns1 contains "${row.syns1.length}" words`);
            /*
            ...
            if (row.syns1 && row.syns1[0]) {
                ...
                for (let syn1 of row.syns1) {
                    let syns_of_syn1 = [];
                    let row1 = await data_word_get_parsed(syn1, "key,root,pos1,pos2,singular,plural,dict,sentiment,list_count,xver,xadj,xadv,xnou,xint,xetc,xpro,xpre");
                    if (row1 && row1.list_count) {
                        if (DEBUG1)
                            global.cconsole.success(`found row1 key="${row1.key}" list_count="${row1.list_count}"`);
                        ...
                        if (DEBUG2)
                            global.cconsole.log("row1 list1", list1);
                        /*
                         * Syns *2
                         */
                        // use list list only if word is positive sentiment - if negative, use all available words
                        if (DEBUG2)
                            global.cconsole.log(`syn1="${syn1}" list contains "${list1.length}" words`);
                        // if (row1["x" + row1.pos1]) {
                        ...
                        for (let syn2 of [row1.key, row1.root, row1.singular, row1.plural, ...list1].filter((str) => !!str)) {
                            let row2 = await data_word_get_parsed(syn2, "key,root,pos1,pos2,singular,plural,acronym,abbreviation,dict,sentiment,list_count");
                            if (row2 && row2.list_count) {
                                ...
                                if (DEBUG2)
                                    global.cconsole.log("row2 list2", list2);
                                /*
                                ...
                                if (row2.singular) {
                                    let row = await data_word_get(row2.singular.toLowerCase(), "key,list_count");
                                    if (row && row.list_count >= 10) {
                                        syns_of_syn1.push("" + row.key);
                                    }
                                }
                                if (row2.plural) {
                                    let row = await data_word_get(row2.plural.toLowerCase(), "key,list_count");
                                    if (row && row.list_count >= 10) {
                                        syns_of_syn1.push("" + row.key);
                                    }
                                }
                                if (row2.abbreviation) {
                                    let row = await data_word_get(row2.abbreviation.toLowerCase(), "key,list_count");
                                    if (row && row.list_count >= 10) {
                                        syns_of_syn1.push("" + row.key);
                                    }
                                }
                                if (row2.acronym) {
                                    let row = await data_word_get(row2.acronym.toLowerCase(), "key,list_count");
                                    if (row && row.list_count >= 10) {
                                        syns_of_syn1.push("" + row.key);
                                    }
                                }
                                ...
                                if (DEBUG2)
                                    global.cconsole.log(`row2 key="${row2.key}", list="${list2.toString()}", sentiment="${row2.sentiment}"`);
                        ...
                        if (DEBUG1)
                            global.cconsole.warn(`not found row for syn1="${syn1}"`);
                    }
                    /*
