...
export default function (inputRow) {
    return new Promise(async (resolve, reject) => {
         ...
         * Get row - from DB
         */
        let row = await data_domain_get_parsed(inputRow.key);
        /*
         * Rebuild syns123
         */
        row.syns1 = [];
        row.syns2 = [];
        row.syns3 = [];
        let syns_of_syns = await domain_syns_of_syns(inputRow.key);
...
async function add_to_list(list, key, limit = 3, use_full_list = false) {
     ...
     */
    let row = await data_word_get_parsed(key, "root,singular,plural,acronym,abbreviation,derivations," + which_list);
