...
export default function (string, options = {}) {
    return new Promise(function (resolve) {
        ...
        setImmediate(function () {
            // start
            if (DEBUG1)
                global.cconsole.info(`autosuggest input "${string}"`);
            let output = {
                source: "bing"
            };
            let suggestions = [];
            try {
                ...
                axios
                    ...
                    if (response.data.suggestionGroups &&
                        ...
                        if (DEBUG1)
                            global.cconsole.log("all", suggestions);
                    ...
                    if (DEBUG1)
                        global.cconsole.log("filtered", suggestions);
                ...
                if (DEBUG1)
                    global.cconsole.error(`bing autosuggest error on "${string}"`, err);
            }
        });
    });
}
