...
export default function (string, options = {}) {
    return new Promise(function (resolve) {
        ...
        setImmediate(function () {
            // start
            if (DEBUG1)
                global.cconsole.info(`GAutocomplete input "${string}"`);
            let output = {
                source: "google"
            };
            let suggestions = [];
            try {
                ...
                axios
                    ...
                    if (response.data && response.data[1] && response.data[1] instanceof Array) {
                        ...
                        if (DEBUG1)
                            global.cconsole.log("GAutocomplete all", suggestions);
                    ...
                    suggestions = suggestions.filter((str) => {
                        ...
                        if ({ asl: 1, net: 1, age: 1, baby: 1, meme: 1, wife: 1, talk: 1, pic: 1, free: 1, crossword: 1 }[last_word]) {
                            return false;
                        }
                    ...
                    if (DEBUG1)
                        global.cconsole.log("GAutocomplete filtered", suggestions);
                ...
                if (DEBUG1)
                    global.cconsole.error(`GAutocomplete error on "${string}"`, err);
            }
        });
    });
}
