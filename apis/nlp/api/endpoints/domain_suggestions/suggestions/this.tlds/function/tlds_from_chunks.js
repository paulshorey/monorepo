...
export default async function () {
    ...
    if (DEBUG1) {
        ...
        global.cconsole.log("this.chunks_dict[this.keys_words[0]].list_count", (this.chunks_dict[this.keys_words[0]] || {}).list_count);
    }
    /*
        ...
        if (row.tlds) {
            if (row.tlds[0] && row.tlds[0][0]) {
                for (let t = 0; t < Math.min(row.tlds[0].length, 5); t++) {
                    ...
                    if (tld)
                        add_to_set(tld, dset1, dset0, 4 * add_rating, ratings);
                }
            }
            if (row.tlds[1] && row.tlds[1][0]) {
                for (let t = 0; t < Math.min(row.tlds[1].length, 5); t++) {
                    ...
                    if (tld)
                        add_to_set(tld, dset2, dset1, 3 * add_rating, ratings);
                }
            }
            if (row.tlds[2] && row.tlds[2][0]) {
                for (let t = 0; t < Math.min(row.tlds[2].length, 5); t++) {
                    ...
                    if (tld)
                        add_to_set(tld, dset3, dset2, 2 * add_rating, ratings);
                }
            }
            if (row.tlds[3] && row.tlds[3][0]) {
                for (let t = 0; t < Math.min(row.tlds[3].length, 5); t++) {
                    ...
                    if (tld)
                        add_to_set(tld, dset4, dset3, 1 * add_rating, ratings);
                }
            }
            break;
        }
        ...
        for (let key in this.chunks_dict) {
            ...
            if (row.tlds) {
                if (row.tlds[0] && row.tlds[0][t]) {
                    let tld = row.tlds[0][t];
                    if (tld)
                        add_to_set(tld, dset2, dset1, 7, ratings);
                }
                if (row.tlds[1] && row.tlds[1][t]) {
                    let tld = row.tlds[1][t];
                    if (tld)
                        add_to_set(tld, dset3, dset2, 5, ratings);
                }
                if (row.tlds[2] && row.tlds[2][t]) {
                    let tld = row.tlds[2][t];
                    if (tld)
                        add_to_set(tld, dset4, dset3, 3, ratings);
                }
                if (row.tlds[3] && row.tlds[3][t]) {
                    let tld = row.tlds[3][t];
                    if (tld)
                        add_to_set(tld, dset5, dset4, 1, ratings);
                }
            }
        }
    }
    ...
    if (DEBUG3)
        global.cconsole.debug("ndoms", ndoms);
    if (DEBUG3)
        global.cconsole.log(new Set([...dset0, ...dset1, ...dset2]).size);
    if (ndoms < 5) {
        this.use_autocomplete = true;
        if (DEBUG3)
            global.cconsole.warn("use autocomplete! get TLDs from context");
    }
    /*
    ...
    if (this.bing_alts.length) {
        for (let alti = 0; alti < this.bing_alts.length || 0; alti++) {
            ...
            let ders = [row.key];
            if (row.plural)
                ders.push(row.plural);
            if (row.singular)
                ders.push(row.singular);
            if (row.root)
                ders.push(row.root);
            if (row.acronym)
                ders.push(row.acronym);
            if (row.abbreviation && typeof row.abbreviation === "string")
                ders.push(row.abbreviation.split(",")[0].trim());
            for (let der of ders) {
                if (tlds_all[der]) {
            ...
            for (let t = 0; t < 10; t++) {
                if (DEBUG3)
                    global.cconsole.log(row.key, row.tlds);
                if (row.tlds) {
                    if (row.tlds[0] && row.tlds[0][t]) {
                        let tld = row.tlds[0][t];
                        if (tld)
                            add_to_set(tld, dset2, dset1);
                    }
                    if (row.tlds[1] && row.tlds[1][t]) {
                        let tld = row.tlds[1][t];
                        if (tld)
                            add_to_set(tld, dset3, dset2);
                    }
                    if (row.tlds[2] && row.tlds[2][t]) {
                        let tld = row.tlds[2][t];
                        if (tld)
                            add_to_set(tld, dset4, dset3);
                    }
                    if (row.tlds[3] && row.tlds[3][t]) {
                        let tld = row.tlds[3][t];
                        if (tld)
                            add_to_set(tld, dset5, dset4);
                    }
                }
            }
        }
