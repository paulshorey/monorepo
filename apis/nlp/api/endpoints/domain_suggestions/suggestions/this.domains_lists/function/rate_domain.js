...
export default function (domain, tlds_ratings) {
    ...
    if (total_alliteration_bonus) {
        ...
        console.log([domain.string, total_alliteration_bonus]);
    }
    /*
     *
     * rhythm
    ...
    // output
    if (DEBUG1)
        global.cconsole.info(`${Math.round(rating)} "${domain.words.toString()}.${tld}"`, first_rating, last_rating, "aa+" + Math.round(alliteration_bonus));
    return rating;
}
