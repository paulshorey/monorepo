...
export default function () {
    ...
    if (bing_alts.length) {
        if (DEBUG1)
            global.cconsole.log("use_autocomplete keys_words:", keys_words);
        if (DEBUG1)
            global.cconsole.log("use_autocomplete bing_alts:", bing_alts);
        for (let alt_str of bing_alts) {
            if (alt_str.length > 30)
                continue;
            if (DEBUG1)
                console.log([alt_str]);
            ...
            if (last_word && stopwords[last_word])
                continue;
