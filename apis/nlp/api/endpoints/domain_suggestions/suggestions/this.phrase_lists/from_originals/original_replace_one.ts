...
export default function () {
  ...
  for (let chunk_i = 0; chunk_i < use_keys.length; chunk_i++) {
    ...
    if (row.aux || row.stop) continue
    if (DEBUG1) global.cconsole.info(row.key, row.pos1, !!row.poss[row.pos1])
    ...
    let syns_dict: any = {}
    // use derivations
    if (row.root) syns_dict[row.root.toLowerCase()] = row.root
    if (row.singular) syns_dict[row.singular.toLowerCase()] = row.singular
    if (row.abbreviation) {
    ...
    if (row.acronym) syns_dict[row.acronym.toLowerCase()] = row.acronym.toLowerCase()
    // only allow plural for last word in phrase
    if (chunk_i === use_keys.length - 1) {
      if (row.plural) syns_dict[row.plural.toLowerCase()] = row.plural
    }
    // use synonyms
    if (row.poss[row.pos1]) {
      ...
      if (DEBUG1) global.cconsole.log("use synonyms", row.poss[pos])
    ...
    let syns_list: string[] = Object.values(syns_dict)
    ...
    for (let syn_i = 0; syn_i <= 30; syn_i++) {
      let syn: string = syns_list[syn_i]
      if (!syn) continue
      syn = syn.toLowerCase()
      /*
   ...
   *
   */
  if (DEBUG0) global.cconsole.log("lists_of_phrases", lists_of_phrases)
  ...
  if (DEBUG0) global.cconsole.log(this.phrase_lists["replace one"])
}
