...
// import {objects_merge} from "@techytools/fn/io/objects";
import nlp from "./nlp"
...
export default async function (
  ...
  if (DEBUG1) global.cconsole.success("keys_words", results.keys_words)
  if (DEBUG1) global.cconsole.success("chunks_dict", Object.keys(results.chunks_dict))
  /*
  ...
  if (length_diff >= 4) {
    ...
    if (DEBUG1) global.cconsole.error(`is_brand because length_diff >=4 (${length_diff})`)
    if (DEBUG1) global.cconsole.error(`results.string_original_nospaces="${results.string_original_nospaces}"`)
    if (DEBUG1) global.cconsole.error(`results.string_nospaces="${results.string_nospaces}"`)
  }
  ...
  if (results.keys_words.length === 1) {
    ...
    if (!row || !row.list_count || (!row.name && row.pos1 === "etc" && row.list_count < 10 && row.key.length >= 3)) {
      ...
      if (DEBUG1) global.cconsole.error(`is_brand because !row for key="${results.keys_words[0]}"`)
    } else if (row.brand) {
      // brand
      results.is_brand = row.key
      if (DEBUG1) global.cconsole.error(`is_brand because row.brand key="${row.key}"`)
    } else {
    ...
    if (!results.is_name && !is_about && row.list_count < 10) {
      ...
      if (DEBUG1) global.cconsole.error(`is_brand because one low list_count word "${results.is_brand}"`)
    }
  }
  if (DEBUG0 && results.is_name) global.cconsole.error("results.is_name", results.is_name)
  if (DEBUG0 && results.is_brand) global.cconsole.error("results.is_brand", results.is_brand)
  if (DEBUG0 && is_about) global.cconsole.error("is_about: nou, ver", results.is_about_nou, results.is_about_ver)
  /*
   *
   * Best 123
   *
  ...
  if (best1) results.best_keys.push(best1)
  if (best2) results.best_keys.push(best2)
  if (best3) results.best_keys.push(best3)
  /*
   *
   * DEFAULT OPTIONS
   *
