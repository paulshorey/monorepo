...
export default function (this: any) {
  if (DEBUG1) global.cconsole.warn("this.phrases combine lists...")
  ...
  while (i_while < i_while_max) {
    ...
    for (let tuple of phrase_lists_tuples) {
      ...
      let list: any = tuple[1]
      if (DEBUG2) global.cconsole.log(`listname:"${listname}", i_while:"${i_while}", n2add:"${n2add}"`)
      ...
      while (i_phrase < i_phrase_max) {
        ...
        for (let word of phrase) {
          if (DEBUG1) global.cconsole.log("add??", phrase, listname)
        ...
        if (phrase_string) {
          ...
          if (DEBUG1) global.cconsole.log("added", phrase, listname)
        }
  ...
  if (DEBUG0) global.cconsole.log(this.phrases)
}
